class SitesImporter
  ...
  ORGANISATIONS_SLUGS = ["eisenia", "grand-lyon-metropole"]

  def import(slug)
    return 'organisation unknown' unless ORGANISATIONS_SLUGS.include? slug
    import_from(slug)
  end

  def import_from(slug)
    data_method = "data_#{slug}".gsub("-", "_")
    csv_data = send(data_method.to_sym)
  ...
  def map_site_data(slug, csv_line)
    send("map_#{slug}_site_data".gsub("-", "_"), csv_line)
  end
